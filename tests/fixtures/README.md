...
- `http/github-releases/` — for the self-update tests: `releases.json`, a release list in the shape
  GitHub sends (an untagged pre-release, a draft, a flagged pre-release, a described 0.13.0 and an
  undescribed 0.12.0), the `lockrot.phar.meta.json` of its 0.13.0, and a 404 body. Every other list
  and description is generated by `tests/Support/GitHubReleases.php`.
- `schema-evolution/` — what earlier releases published and wrote, for `SchemaEvolutionTest`.
  ...
  0.9.0 to 0.13.0, taken from its tag (0.13.0's from its release commit, which the tag then points
  at; the same content as the verified release PHAR carries, compacted there); the test pins their
  sha256. `<version>/` holds, per recorded release, the
