...
src/Composer/SelfUpdateCommand.php:122 FalseValue (`\Phar::running(false)` → `\Phar::running(true)`) — the
...
src/Composer/SelfUpdateCommand.php:154 ConcatOperandRemoval (drops the closing `'</error>'`) — Symfony's
...
src/Composer/SelfUpdateCommand.php:158 ConcatOperandRemoval (drops the closing `'</error>'`) — the same, on the
