...
test("libyearsAtZero reads the value, not the 0.0 it rounds to", () => {
    ...
    assert.strictEqual(lib.libyearsAtZero({ libyears: 0, version: "v1.3.0-beta1" }, meta), "not behind the newest stable, v1.2.4", "a pre-release above the newest");
    assert.strictEqual(lib.libyearsAtZero({ libyears: 0, version: "v1.1.9" }, meta), "not behind the newest stable, v1.2.4", "a backport released after the newest is older, and not behind in time");
