...
"wallabag is 151.5 libyears behind" does. The libyear (libyear.com, on the version-release-date
...
against the newest stable, and leaves the undated unmeasured. On wallabag that is 94.5 against
151.5. The `direct_requirements` subtotal is the nearest number to that tool's — compare under
